...
export async function buildAuthUserPayload(
  ...
  let providerBio: string | null | undefined;

  if (profile.role === "provider") {
    const { data: provider } = await serviceClient
      .from("providers")
      .select("status, skills, experience_years, bio")
      .eq("user_id", userId)
      .maybeSingle();
    providerStatus = (provider?.status as string | undefined) ?? null;
    providerSkills = (provider?.skills as string[] | undefined) ?? [];
    providerExperienceYears = (provider?.experience_years as number | undefined) ?? 0;
    providerBio = (provider?.bio as string | null | undefined) ?? null;
  }
